...
public class AccountDaoImpl implements AccountDao {
	...
	private final String FIND_ACCOUNT = "SELECT id, name, password FROM account WHERE name = ? AND password = ?";
	...
	public AccountBean findAccount(String name, String password) {
		...
		try {
			result = qr.query(FIND_ACCOUNT, new BeanHandler<AccountBean>(AccountBean.class), new Object[] {name, password});
			if (result != null) {
				List<RoleBean> roles = qr.query(FIND_ROLES_BY_ACCOUNT_ID, new BeanListHandler<RoleBean>(RoleBean.class), new Object[] {result.getId()});
	...
	public List<AccountBean> findAccountsByRange(int startRowId, int endRowId) {
		...
		try {
			result = qr.query(FIND_ACCOUNTS_BY_RANGE, new BeanListHandler<AccountBean>(AccountBean.class), new Object[] {startRowId, endRowId});
		} catch (SQLException e) {
			throw new DaoException(e);
		}
		return result;
	...
	public List<AccountBean> findAllAccounts() {
		List<AccountBean> result = null;
		try {
			result = qr.query(FIND_ALL_ACCOUNTS, new BeanListHandler<AccountBean>(AccountBean.class));
		} catch (SQLException e) {
			throw new DaoException(e);
		}
		return result;
