		...
		<c:if test="${not empty requestScope.page.records}">
			<table class="list">
				...
				<c:forEach var="menu" items="${requestScope.page.records}" varStatus="vs">
					<tr>
						...
						<td>
							<a href="${uri}?action=show&view=mgrUpdateMenu&id=${menu.id}"><fmt:message key="menu.ops.update" bundle="${bundle}" /></a>
							<a href="javascript: delMenu('${menu.id}')"><fmt:message key="menu.ops.delete" bundle="${bundle}" /></a>
						</td>
					</tr>
				</c:forEach>
			</table>
			<p id="page">
				<c:if test="${requestScope.page.prevPageId eq -1}">
					<fmt:message key="menu.prevPage" bundle="${bundle}"/>
				</c:if>
				<c:if test="${requestScope.page.prevPageId ne -1}">
					<a href="${uri}?action=show&view=mgrMenu&pageId=${requestScope.page.prevPageId}"><fmt:message key="menu.prevPage" bundle="${bundle}"/></a>
				</c:if>
				...
				<c:if test="${requestScope.page.nextPageId eq -1}">
					<fmt:message key="menu.nextPage" bundle="${bundle}"/>
				</c:if>
				<c:if test="${requestScope.page.nextPageId ne -1}">
					<a href="${uri}?action=show&view=mgrMenu&pageId=${requestScope.page.nextPageId}"><fmt:message key="menu.nextPage" bundle="${bundle}"/></a>
				</c:if>
			</p>
		</c:if>
	</form>
